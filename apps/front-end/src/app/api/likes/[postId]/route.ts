...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${backendUrl}/likes/${params.postId}`, {
      headers: {
    ...
    return NextResponse.json(
      { success: false, message: '좋아요 목록 조회에 실패했습니다.' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: '좋아요가 삭제되었습니다.' });
  } catch (error) {
